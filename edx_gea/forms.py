...
class UploadAssessmentFileForm(forms.Form):
    ...
    def clean_assessment_file(self):
        ...
        for line, row in enumerate(assessment_file):
            if line > self.gea_xblock.max_assessment_file_lines:
                self.add_form_error(line, _("The csv file has more than %s lines.") % self.gea_xblock.max_assessment_file_lines)
                return
            ...
            if row['score']:
                ...
                self.add_form_error(line, _("Student needs a score to be graded."))

    def is_user(self, line, username):
        try:
            self.gea_xblock.usernames[username] = User.objects.get(username=username)
        except User.DoesNotExist:
            self.add_form_error(line, _("User %s does not exist.") % username)
        else:
            return True
    ...
    def check_user_enrollment(self, line, username):
        if not CourseEnrollment.is_enrolled(self.gea_xblock.usernames[username],
            ...
            self.add_form_error(line, _("User %s is not enrolled to this course.") % username)

    def check_score(self, line, score):
        try:
            score = int(score)
        except ValueError:
            self.add_form_error(line, _("Score %s is not an integer.") % score)
        else:
            if not 0 <= score <= self.gea_xblock.max_score():
                self.add_form_error(line, _("Score %s is outside score limits.") % score)
    ...
    def add_form_error(self, line, error_description):
        ...
        error_msg = _("Line %(line)s: %(error_description)s") % {'line' : line, 'error_description' : error_description}
